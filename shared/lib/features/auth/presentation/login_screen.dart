...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  Future<void> _submit() async {
    ...
    // Nouveau compte : compléter ville, quartier et profil avant d'entrer.
    context.go(_registering ? '/city' : ref.read(appTargetProvider).homeFor(ref.read(authProvider).user!.role));
  }
        ...
        children: [
          ...
          Center(
            child: TextButton.icon(
              onPressed: auth.loading ? null : () => context.go(_registering ? '/register' : '/register?login=1'),
              icon: const Icon(Icons.phone_android_outlined, size: 18),
              label: Text(_registering ? 'S’inscrire avec mon numéro de téléphone' : 'Me connecter avec mon numéro et mon code'),
            ),
          ),
          const SizedBox(height: 12),
          const Text(
