...
class AuthNotifier extends Notifier<AuthState> {
  ...
  /// Inscription téléphone : le code à 6 chiffres est choisi par l'utilisateur.
  Future<bool> registerPhone(String phone, String pin, {String name = ''}) async {
    state = state.copyWith(loading: true, clearError: true);
    try {
      final user = await ref.read(authRepositoryProvider).registerPhone(phone: phone, pin: pin, name: name);
      state = AuthState(user: user);
      return true;
    } catch (e) {
      state = state.copyWith(loading: false, error: e.toString());
  ...
  Future<bool> loginPhone(String phone, String pin) async {
    state = state.copyWith(loading: true, clearError: true);
    try {
      final user = await ref.read(authRepositoryProvider).loginPhone(phone: phone, pin: pin);
      state = AuthState(user: user);
      return true;
    } catch (e) {
      state = state.copyWith(loading: false, error: e.toString());
