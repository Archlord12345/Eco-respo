...
class AppwriteAuthRepository implements AuthRepository {
  ...
  // ---------------------------------------------------------------- téléphone
  //
  // Appwrite n'accepte pas nativement « téléphone + code » sans passerelle
  // SMS. On dérive donc un identifiant email technique du numéro et un mot de
  // passe fort (SHA-256 du couple numéro/code) : l'utilisateur ne retient que
  // son numéro et son code à 6 chiffres, aucun message n'est envoyé.

  static const _phoneDomain = 'phone.eco-responsable.cm';

  static String phoneToEmail(String phone) => '${phone.replaceAll(RegExp(r'\D'), '')}@$_phoneDomain';

  static String pinToPassword(String phone, String pin) {
    final digits = phone.replaceAll(RegExp(r'\D'), '');
    return sha256.convert(utf8.encode('eco-responsable|$digits|$pin')).toString();
  }

  static void _checkPin(String pin) {
    if (!RegExp(r'^\d{6}$').hasMatch(pin)) {
      throw const AuthFailure('Le code doit contenir exactement 6 chiffres.');
    }
  }
  ...
  Future<AppUser> registerPhone({required String phone, required String pin, String name = ''}) async {
    ...
    final password = pinToPassword(phone, pin);
    try {
      final created = await _account.create(
        userId: ID.unique(),
        email: email,
      ...
      try {
        ...
        // Le numéro reste enregistré dans le profil (table users) ci-dessous.
      }
      final profile = _defaultProfile(created.$id, name: name, phone: phone);
      await _tables.createRow(
        databaseId: _db,
        tableId: _table,
        rowId: created.$id,
        data: profile.toMap(),
        permissions: _ownerPermissions(created.$id),
      );
      return profile;
    } on AppwriteException catch (e) {
      throw AuthFailure(
        e.code == 409
            ? 'Ce numéro possède déjà un compte. Connectez-vous avec votre code à 6 chiffres.'
  ...
  Future<AppUser> loginPhone({required String phone, required String pin}) async {
    ...
    try {
      ...
      throw AuthFailure(
        e.code == 401 ? 'Numéro ou code à 6 chiffres incorrect.' : (e.message ?? 'Connexion impossible'),
        code: e.code?.toString(),
      );
    }
  }
  ...
  Future<void> changePin({required String phone, required String oldPin, required String newPin}) async {
    ...
    try {
      await _account.updatePassword(
        password: pinToPassword(phone, newPin),
        oldPassword: pinToPassword(phone, oldPin),
      );
    } on AppwriteException catch (e) {
      throw AuthFailure(e.code == 401 ? 'Ancien code incorrect.' : (e.message ?? 'Changement impossible'), code: e.code?.toString());
    }
  }
}
